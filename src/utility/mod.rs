...
mod tests {
  ...
  #[test]
  fn test_write_be_u64() {
    let mut buffer = [0; 8];

    assert!((&mut buffer[..]).write_be_u64(0x0123456789abcdef).is_ok());
    assert_eq!(buffer, [0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef]);

    assert!((&mut buffer[..]).write_be_u64(0x3f9348cda3b8d022).is_ok());
    assert_eq!(buffer, [0x3f, 0x93, 0x48, 0xcd, 0xa3, 0xb8, 0xd0, 0x22]);

    assert!((&mut buffer[..]).write_be_u64(0x0f1e2d3c4b5a6978).is_ok());
    assert_eq!(buffer, [0x0f, 0x1e, 0x2d, 0x3c, 0x4b, 0x5a, 0x69, 0x78]);
  }
}
