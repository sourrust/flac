...
mod tests {
  ...
  #[test]
  fn test_crc16() {
    assert!(crc16(&[0x00, 0x0a]) == 0x003c, "2 Byte hash (0x003c)");
    assert!(crc16(&[0x80, 0x05, 0x80, 0xbb]) == 0x03e8,
            "4 Byte hash (0x03e8)");
    assert!(crc16(&[0x00, 0x6c, 0x01, 0x92, 0x02, 0xd0]) == 0x02a8,
            "6 Byte hash (0x02a8)");
    assert!(crc16(&[0x00, 0xaa]) == 0x03fc, "2 Byte hash (0x03fc)");
    assert!(crc16(&[0x80, 0xb1, 0x83, 0xa9]) == 0x0014,
            "4 Byte hash (0x0014)");
    assert!(crc16(&[0x80, 0x1b, 0x80, 0xeb, 0x03, 0x90]) == 0x0000,
            "6 Byte hash (0x0000)");
  }
}
