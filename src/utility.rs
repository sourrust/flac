...
mod tests {
  ...
  fn test_crc8() {
    ...
    assert_eq!(crc8(&[0x94]), 0xe5);
    assert_eq!(crc8(&[0x36, 0xa7]), 0xfb);
    assert_eq!(crc8(&[0x4f, 0x9e, 0x4b]), 0x00);
  }

  #[test]
  fn test_crc16() {
    assert_eq!(crc16(&[0x00, 0x0a]), 0x003c);
    assert_eq!(crc16(&[0x80, 0x05, 0x80, 0xbb]), 0x03e8);
    assert_eq!(crc16(&[0x00, 0x6c, 0x01, 0x92, 0x02, 0xd0]), 0x02a8);
    assert_eq!(crc16(&[0x00, 0xaa]), 0x03fc);
    assert_eq!(crc16(&[0x80, 0xb1, 0x83, 0xa9]), 0x0014);
    assert_eq!(crc16(&[0x80, 0x1b, 0x80, 0xeb, 0x03, 0x90]), 0x0000);
  }
}
